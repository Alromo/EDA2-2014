...
public class Principal2 {
	...
	public static void main(String[] args) {		
		...
		leerArchivo("C:" + File.separator +/*"WORKSPACES"+ File.separator + "PracticasEDA2" +File.separator +*/"PruebaAlgoritmia" + File.separator + "DivideYVenceras"+ File.separator+ "test4.txt");
		ArrayList<Punto> resultadoDyV=divideYVenceras(puntosUsados);			//Llamada al método divide y vencerás	
		if(resultadoDyV.size()!=1000){																	
			while(resultadoDyV.size()<1000){												//En esta parte de código controlamos que el resultado tenga, en efecto, el 1% de los 100,000
				puntosUsados.removeAll(resultadoDyV);										//puntos especificados. Primero comprobamos que es distinto del número pedido.																																												 					
				...
				resultadoDyV.addAll(resultado2);
				Collections.sort(resultadoDyV);	
				//System.out.println("Realiza divide");
			}														
		}																				
		while(resultadoDyV.size()>1000){													//Si se tienen más puntos, comenzará el proceso de eliminación de puntos sobrantes.
			if(flag){																	//Al ser las fronteras las zonas menos significativas del problema, iremos eliminando elementos de ellas, uno a uno
				resultadoDyV.remove(0);													//y de forma alternada. Primero un lado, despues otro, y esto lo controlaremos con la variable de control 'flag'
				flag=false;
			}else{
				resultadoDyV.remove(resultadoDyV.size()-1);
				flag=true;
			}
		}
		escribirArchivo("C:" + File.separator +/*"WORKSPACES"+ File.separator + "PracticasEDA2" +*/File.separator +"PruebaAlgoritmia" + File.separator + "DivideYVenceras"+ File.separator+ "Sol4.txt", resultadoDyV);
		long fin = System.currentTimeMillis();
		long tiempoFinal = fin - inicio;											//Finalmente escribiremos el archivo de salida con la solución especificada
		System.out.println(tiempoFinal);
		ArrayList<Punto> resultadoVoraz=Voraz(resultadoDyV, puntosLeidos);
		escribirArchivo("C:" + File.separator +/*"WORKSPACES"+ File.separator + "PracticasEDA2" +*/File.separator +"PruebaAlgoritmia" + File.separator + "DivideYVenceras"+ File.separator+ "SolVor4.txt", resultadoVoraz);
	...
	private static ArrayList<Punto> Voraz(ArrayList<Punto> resultadoAnterior,
		ArrayList<Punto> puntosTotales) {
		if(resultadoAnterior.size()==0){
			return null;
		}
		ArrayList<Punto> resultado= new ArrayList<Punto>();			
		for(int i=0;i<resultadoAnterior.size();i++){
			Punto puntoSeleccionado = seleccion(resultadoAnterior.get(i));
			if(puntoSeleccionado!=null){								
				resultado.add(puntoSeleccionado);
			}
		}
		if(resultado.size()<=5){
			return resultado;
		...
		ArrayList<Punto> resultadoFinal=new ArrayList<Punto>();
		for(int i=resultado.size()-1; i>0; i--){
			resultadoFinal.add(resultado.get(i));
			if(resultadoFinal.size()==5){
				break;
			}
		}					
	...
	private static Punto seleccion(Punto punto) {
		ArrayList<Integer> distancia = new ArrayList<Integer>();
		ArrayList<Punto> puntosAComprobar = new ArrayList<Punto>();
		if(puntosLeidos.indexOf(punto)>0){
			for(int i=1;i<=m;i++){
				if(puntosLeidos.indexOf(punto)-i<=0){
					puntosAComprobar.add(puntosLeidos.get(puntosLeidos.indexOf(punto)-i));
					break;
				}
				puntosAComprobar.add(puntosLeidos.get(puntosLeidos.indexOf(punto)-(i)));
			}
		}
		if(puntosLeidos.indexOf(punto)<puntosLeidos.size()-1){
			for(int i=1;i<=m;i++){
				if(puntosLeidos.indexOf(punto)+i>=puntosLeidos.size()-1){
					puntosAComprobar.add(puntosLeidos.get(puntosLeidos.indexOf(punto)+(i)));
					break;
				}
				puntosAComprobar.add(puntosLeidos.get(puntosLeidos.indexOf(punto)+i));
		...
		Collections.sort(puntosAComprobar);
		for(int i=0;i<puntosAComprobar.size();i++){
			distancia.add(calcularDistancia(punto,puntosAComprobar.get(i)));
		}
		while (puntosAComprobar.size()>m){
			puntosAComprobar.remove(distancia.indexOf(Collections.max(distancia)));
			distancia.remove(Collections.max(distancia));
		}	
		double media=0;
		for(int i=0; i<puntosAComprobar.size(); i++){
			media=media+puntosAComprobar.get(i).getY();
		}
		media=media/m;
		punto.setDiferencia((int) Math.abs(media-punto.getY()));
	...
	private static Integer calcularDistancia(Punto punto, Punto punto2) {
		int valorX=(int) Math.pow(punto.getX()-punto2.getX(), 2);
		int valorY=(int) Math.pow(punto.getY()-punto2.getY(), 2);
		return (int)Math.sqrt(valorX+valorY);
	}
